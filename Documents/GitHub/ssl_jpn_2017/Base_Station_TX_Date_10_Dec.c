...
void ssl_LED_init()
{
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOE, ENABLE);  //Initialisation of clock
      GPIO_InitTypeDef y;     //De6fining a structure named 'x'
...
void USART1_IRQHandler()
{
	if(USART_GetITStatus(USART1, USART_IT_RXNE)== SET)
	{
		...
		if(count==0)
		{
			dataInUSART[0] = inData;
			count++;
		}
		else if(dataInUSART[0]==127)
		{
			dataInUSART[count]=inData;
			count++;
		}
...
int main(void) {
    ...
    //LED_initialise();
    //SystemInit(); /0figures the system clock, system clock source, PLL Multiplier and Divider factors, AHB/APBx prescalers
    ssl_LED_init();
...
//					dataInUSART[count]= USART_ReceiveData(USART1);
//				}
//				else if(dataInUSART[0]==team_id)//127 is Team_ID
//				{
//					dataInUSART[count] = USART_ReceiveData(USART1);
//				}
//
...
//        }
//
//            if(count==32)   // >7 is used for 8 bytes || >31 transmits a packet of 32 bytes
//            {
//                count=0;
//                for(i=0;i<32;i++){
//                    dataOut[i] = dataInUSART[i];}
